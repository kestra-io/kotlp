...
void otel_resource(sb *s, const kotlp_config *cfg, pid_t child_pid) {
    ...
    sb_putc(s, ',');
    otel_attr_str(s, "telemetry.sdk.name", "kotlp");
    sb_putc(s, ',');
    otel_attr_str(s, "telemetry.sdk.language", "c");
    sb_putc(s, ',');
    otel_attr_str(s, "telemetry.sdk.version", KOTLP_VERSION);
    sb_putc(s, ',');
    otel_attr_str(s, "host.name", kotlp_hostname());
    sb_putc(s, ',');
